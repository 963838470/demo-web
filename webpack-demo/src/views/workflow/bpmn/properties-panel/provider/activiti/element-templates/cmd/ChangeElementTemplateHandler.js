...
var createActivitiProperty = require('../CreateHelper').createActivitiProperty,
  ...
  createActivitiInWithBusinessKey = require('../CreateHelper')
    .createActivitiInWithBusinessKey,
  createActivitiExecutionListenerScript = require('../CreateHelper')
    .createActivitiExecutionListenerScript,
  createActivitiFieldInjection = require('../CreateHelper')
    .createActivitiFieldInjection
...
function ChangeElementTemplateHandler(modeling, commandStack, bpmnFactory) {
  ...
  function updateExecutionListener(element, newTemplate, context) {
    var newExecutionListeners = createActivitiExecutionListeners(
        newTemplate,
        bpmnFactory
      ),
...
function createActivitiFieldInjections(template, bpmnFactory) {
  ...
  template.properties.forEach(function(p) {
    ...
    if (bindingType === 'activiti:field') {
      injections.push(
        createActivitiFieldInjection(binding, p.value, bpmnFactory)
      )
    }
  })
...
function createActivitiInOut(template, bpmnFactory) {
  ...
  template.properties.forEach(function(p) {
    ...
    if (bindingType === 'activiti:in') {
      ...
      inOuts.push(
        createActivitiInWithBusinessKey(binding, p.value, bpmnFactory)
      )
    }
  })
  ...
  template.properties.forEach(function(p) {
    ...
    if (bindingType === 'activiti:executionListener') {
      executionListener.push(
        createActivitiExecutionListenerScript(binding, p.value, bpmnFactory)
      )
    }
  })
