...
function getActivitiInWithBusinessKey(element) {
  ...
  var activitiInParams = extensionElementsHelper.getExtensionElements(
    bo,
    'activiti:In'
  )
  if (activitiInParams) {
  ...
  function getDelegateVariableMappingType(element) {
    ...
    var boVariableMappingClass = bo.get('activiti:variableMappingClass'),
      boVariableMappingDelegateExpression = bo.get(
        'activiti:variableMappingDelegateExpression'
      )
  ...
  entries.push(
    entryFactory.selectBox({
      ...
      get: function(element, node) {
        ...
        return {
          mapDecisionResult:
            bo.get('activiti:mapDecisionResult') || 'resultList'
        }
      },
