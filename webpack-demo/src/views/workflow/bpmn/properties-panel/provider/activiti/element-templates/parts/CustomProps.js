...
function setPropertyValue(element, property, value, bpmnFactory) {
  ...
  if (bindingType === ACTIVITI_PROPERTY_TYPE) {
    if (scope) {
      ...
      activitiProperties = findExtension(
        extensionElements,
        'activiti:Properties'
      )
    }
    ...
    if (!activitiProperties) {
      activitiProperties = elementHelper.createElement(
        'activiti:Properties',
        null,
        bo,
        bpmnFactory
      ...
      existingFieldInjections.forEach(function(item) {
        if (item.name === binding.name) {
          newFieldInjections.push(
            createActivitiFieldInjection(binding, value, bpmnFactory)
          )
        } else {
