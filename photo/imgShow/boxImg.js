$(document).on("click", ".zoomImg", function() {
  //给body添加弹出层的html
  var len = $(".zoomImg").length;
  var arrPic = new Array(); //定义一个数组
  for (var i = 0; i < len; i++) {
    arrPic[i] = $(".zoomImg")
      .eq(i)
      .prop("src"); //将所有img路径存储到数组中
  }
  ...
  var img_index = $(".zoomImg").index(this); //获取点击的索引值
  var num = img_index;
  ...
  function showImg() {
    ...
    if (initial_width > initial_height) {
      ...
      $(".auto-img-center img").css(
        "margin-top",
        -(last_imgHeight - box_height) / 2
      );
    } else {
